...
const FEATURES = [
  {
    ...
    body: 'Real terminals in a grid, grouped by project folder. Watch six agents work at once, jump into any of them, broadcast an instruction to all.',
  },
  {
    title: 'Deploy a whole team',
    body: 'One click puts an orchestrator, frontend, backend, QA, audit and database engineer on a project, each on its own git worktree, each with its own model and instructions.',
  },
  {
    title: 'Any CLI, any model',
    body: 'Claude, Codex, Gemini, Grok, OpenCode, Deepseek, Kimi, MiniMax and more. Pick the provider, the model and the effort level per agent.',
  },
  {
    title: 'One shared memory',
    body: 'Native project memory, gbrain and Honcho are wired into every agent, so what one session learns the next one already knows.',
  },
  {
    title: 'Driven by Hermes',
    body: 'Connect your self-hosted gateway (local, SSH, remote or cloud). Its cron jobs dispatch work to your agents and its kanban board is right here.',
  },
  {
    title: 'Yours, locally',
    body: 'A desktop app talking to CLIs on your machine. No accounts, no cloud in the middle, no telemetry.',
  },
];
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-bg text-ink">
      ...
      {/* Hero */}
      <section className="max-w-5xl mx-auto px-6 pt-16 pb-20 border-b border-line">
        ...
        <p className="text-ink-soft text-[15px] leading-relaxed max-w-xl mb-9">
          ...
          click, a memory they all share, and your Hermes gateway driving the schedule.
        </p>
      ...
      <section className="max-w-5xl mx-auto px-6 py-16 border-b border-line">
        <div className="border border-line bg-surface">
          <div className="flex items-center gap-3 px-4 py-2.5 border-b border-line">
            <span className="font-mono text-xs text-ink">Dorothy-fix</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-line">
            {[
              { name: 'orchestrator', tone: 'text-ok', lines: ['delegate_task frontend "fix scroll lock"', 'frontend  running   feat/frontend', 'backend   merged  main'] },
              { name: 'frontend', tone: 'text-accent', lines: ['vitest run --changed', '184 passed (184)', 'apply patch to TerminalGrid.tsx? (y/n)'] },
              { name: 'backend', tone: 'text-ok', lines: ['npm run build', 'electron  tsc clean', 'watching...'] },
              { name: 'qa', tone: 'text-ink-muted', lines: ['661 passed (661) in 4.2s', 'coverage  87.3%', ''] },
            ].map(p => (
      ...
      {/* Features */}
      <section id="features" className="max-w-5xl mx-auto px-6 py-16 border-b border-line">
      ...
      {/* How */}
      <section id="how" className="max-w-5xl mx-auto px-6 py-16 border-b border-line">
        ...
        <ol className="space-y-6">
          {[
            ...
            ['Let them work', 'Terminals stream live. The orchestrator delegates, you review, nothing starts without you asking.'],
            ['Wire in Hermes', 'Your gateway schedules the recurring work and its kanban board drives what the team picks up next.'],
          ].map(([t, d], i) => (
      ...
      <section id="download" className="max-w-5xl mx-auto px-6 py-20 text-center">
        ...
        <p className="text-ink-soft text-[15px] mb-8 max-w-md mx-auto">
          macOS desktop app. Free and open source.
        </p>
